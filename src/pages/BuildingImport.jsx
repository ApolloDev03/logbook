...
export default function BuildingImport() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${apiUrl}/auth/import_buildings_csv`,
        formData,
        {
          headers: {
      ...
      toast.error(
        error?.response?.data?.message || "Failed to import buildings."
      );
    } finally {
      setLoading(false);
    }
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">
                Download the sample template, fill building details and upload
                the completed file.
              </p>
            </div>
        ...
        <div className="p-6">
          <div className="mx-auto max-w-2xl rounded-xl border border-dashed border-gray-300 bg-gray-50 p-8 dark:border-gray-700 dark:bg-gray-900">
            ...
            <div className="mt-8 flex justify-end gap-3">
              ...
              >
                {loading ? "Importing..." : "Import Building"}
              </button>
            </div>
          </div>
        </div>
